...
def run_pipeline(companies_input: List[Dict[str, str]]):
    ...
    for input_data in companies_input:
        ...
        target_email = "etedeloro.pt@gmail.com" # Forçando email para testes
        # target_email = input_data.get("email", "etedeloro.pt@gmail.com") # Linha original comentada
        ...
        if "target_persons" in classified_data and len(classified_data["target_persons"]) > 1:
            print("Outras opções encontradas:")
            for i, p in enumerate(classified_data["target_persons"][1:], 1):
                print(f"  {i}. {p['name']} ({p['role']}) - Score: {p.get('score', 0)}")
